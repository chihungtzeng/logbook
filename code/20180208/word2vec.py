...
try:
    ...
    def train_batch_sg(model, sentences, alpha, work=None, compute_loss=False):
        ...
        for w in model.wv.vocab:
            print(w)
            for key in model.wv.vocab[w].__dict__:
                value = model.wv.vocab[w].__dict__[key]
                print(key, value)
        for sentence in sentences:
            ...
            for pos, word in enumerate(word_vocabs):
                ...
                for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                    ...
                    if pos2 != pos:
                        print(u"pos: {} pos2: {}".format(pos, pos2))
                        train_sg_pair(
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    #
    # Args:
    # word(string) -- Target word (string)
    # context_index(int) -- The input word index.
    #
    # Comment: It is strange that |word| and |context_index| have different types.
    #
    # The notation below is used in the following comments.
    # V: vocabulary
    # |V|: the size of vocabulary, i.e., the number of unique words.
    # d: The word embedding (vector) dimension
    # k: The number of negative samples.

    # pudb.set_trace()
    print("-" * 30)
    print(u"train_sg_pair: word={} context_index={}".format(word, context_index))
    if context_vectors is None:
        if is_ft:
            context_vectors_vocab = model.wv.syn0_vocab
            context_vectors_ngrams = model.wv.syn0_ngrams
        else:
            # context_vectors is a numpy.ndarray, with shape |V| * d.
            # Each row of it corresponds to the word embedding.
            #
            # Comment: 1. context_vectors will be updated after training a sg-pair.
            #          2. I have no idea why the variable is named sync0.
            #          3. context_vectors should be the hidden layer (I guess).
    ...
    if context_locks is None:
        if is_ft:
            ...
            # context_locks is a numpy.ndarray, with shape |V| * 1.
            # The initial value for each element in context_locks is 1.
            context_locks = model.syn0_lockf

    print("context_vectors:")
    print(context_vectors)

    org_context_vectors = deepcopy(context_vectors)
    print("model.syn1neg:")
    print(model.syn1neg)
    org_syn1neg = deepcopy(model.syn1neg)
    ...
    if is_ft:
        ...
        # l1: The embedding of the input word embedding
        l1 = context_vectors[context_index]  # input word (NN input/projection layer)
        lock_factor = context_locks[context_index]

    print("l1: {}".format(l1))
    print("lock_factor: {}".format(lock_factor))
    # neu1e: vector of size d, initially all 0.
    #        neu1e propagates errors from output -> hidden
    ...
    if model.negative:
        ...
        while len(word_indices) < model.negative + 1:
            # Randomly select |negative| words that are different from |predict_word|.
            # Each of word_indices[1:] is not necessarily unique.
            # word_indices is of size (k+1).
            w = model.cum_table.searchsorted(model.random.randint(model.cum_table[-1]))
            if w != predict_word.index:
                word_indices.append(w)
        print("word_indices: {}".format(word_indices))
        # l2b is a matrix with dimension (k+1)*d, each of its row is based on |word_indices|.
        # Comment: l2b seems the weights of the output layer.
        l2b = model.syn1neg[word_indices]  # 2d matrix, k+1 x layer1_size
        print("l2b: {}".format(l2b))
        prod_term = dot(l1, l2b.T)
        print("prod_term= {}".format(prod_term))
        # fb seems to bethe output of the network for this sg pair.
        # fb is a vector of dimension (k+1)
        #   Comment: expid is logisitc function, 1/(1 + e^(-x)).
        fb = expit(prod_term)  # propagate hidden -> output
        print("fb: {}".format(fb))
        # (1)
        # model.neg_labels is a vector of dimension (k+1). The first element is 1 and the rest is 0.
        # model.neg_labels[0] == 1 probably means a positive sample, while others mean negative samples.
        # (2)
        # gb seems to be the error of the output
        # gb is a vector of size (k+1)
        gb = (model.neg_labels - fb) * alpha  # vector of error gradients multiplied by the learning rate
        print("model.neg_labels: {}".format(model.neg_labels))
        print("gb: {}".format(gb))
        if learn_hidden:
            # update the output layer weights.
            #
            # After updating the output layer weight,
            # if the upcomming sg pair and |word_indices| are the same to the ones in this function call,
            # The output layer would produce [1, 0, 0, 0,...].
            # The outer product here may be the shortcut for computation.
    ...
    print("l1: {}".format(l1))
    print("context_vectors diff:")
    print(context_vectors - org_context_vectors)
    print("model.syn1neg diff:")
    print(model.syn1neg - org_syn1neg)
    print("+" * 50)
    return neu1e
...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        """
        # inits 為 thread_private_mem
        #   (array([0., 0., 0., 0.], dtype=float32), array([0., 0., 0., 0.], dtype=float32))
        #
        work, neu1 = inits
        tally = 0
        if self.sg:
